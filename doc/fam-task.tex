...
(define (display-event event)
  (let ((mp (fam-event-monitored-path event))
    ...
    (display (format "~A: ~A while monitoring ~A (~A)~%" p type mp time))))
...
(display (format "Monitoring using ~A started~%"
                 (if (fam-use-native?) "scheme FAM" "FAM/Gamin daemon")))

(let loop ()
  (display "(a)dd, (r)emove, (s)uspend, r(e)sume, (q)uit: ")
  (let ((op (read)))
    (if (memq op '(e a r s))
        (let ((path (begin (display "Path: ") (read-string))))
          (if (case op
                ((a) (fam-task-add-path ft path display-event))
                ((r) (fam-task-remove-path ft path))
                ((s) (fam-task-suspend ft path))
                ((e) (fam-task-resume ft path)))
              (display "OK")
              (display "KO"))
          (newline)))
    (if (eq? op 'q) (fam-task-stop ft) (loop))))
...
that happened when it was dead. The parameter \scm{fam-use-native?}
